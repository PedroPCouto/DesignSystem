...
- Tabelas: `TB_NOME_DA_TABELA`
- Colunas (prefixos):
  - `NR_` — Números (ex.: `NR_QUANTIDADE`)
  - `VL_` — Valores (ex.: `VL_PRECO`)
  - `NM_` — Nomes (ex.: `NM_NOME`)
  - `DS_` — Descrições (ex.: `DS_DESCRICAO`)
  - `CD_` — Códigos/IDs (ex.: `CD_CODIGO`)
...
#### TB_PREFERENCIA
- PK: `CD_PREFERENCIA` (BIGSERIAL)
- `NM_TEMA` VARCHAR(50)
- `NM_IDIOMA` VARCHAR(20)
- `DS_NOTIFICACAO_HABILITADA` BOOLEAN
- `DS_LOCAL_ACESSIBILIDADE` BOOLEAN

#### TB_CATEGORIA
- PK: `CD_CATEGORIA` (BIGSERIAL)
- `NM_NOME` VARCHAR(200)

#### TB_PREFERENCIA_CATEGORIA
- PK: (`CD_PREFERENCIA`, `CD_CATEGORIA`)
- `CD_PREFERENCIA` BIGINT FK → TB_PREFERENCIA(`CD_PREFERENCIA`)
- `CD_CATEGORIA` BIGINT FK → TB_CATEGORIA(`CD_CATEGORIA`)

#### TB_ENDERECO
- PK: `CD_ENDERECO` (BIGSERIAL)
- `NM_LOGRADOURO` VARCHAR(255)
- `NR_NUMERO` VARCHAR(30)
- `DS_COMPLEMENTO` VARCHAR(100)
- `NM_BAIRRO` VARCHAR(100)
- `NM_CIDADE` VARCHAR(100)
- `DS_PONTO_REFERENCIA` VARCHAR(150)

#### TB_METADATA
- PK: `CD_METADATA` (BIGSERIAL)
- `DT_CREATED_AT` TIMESTAMPTZ
- `DT_UPDATED_AT` TIMESTAMPTZ
- `DT_DELETED_AT` TIMESTAMPTZ
- `DS_ATIVO` BOOLEAN

#### TB_USUARIO
- PK: `CD_USUARIO` (BIGSERIAL)
- `NM_NOME` VARCHAR(255)
- `NM_EMAIL` VARCHAR(255) UNIQUE
- `DS_SENHA` VARCHAR(255)
- `DT_NASCIMENTO` DATE
- `CD_METADATA` BIGINT FK → TB_METADATA(`CD_METADATA`)
- `CD_ENDERECO` BIGINT FK → TB_ENDERECO(`CD_ENDERECO`)
- `NR_TELEFONE` VARCHAR(30)

#### TB_LOCAL
- PK: `CD_LOCAL` (BIGSERIAL)
- `DS_PUBLICO` BOOLEAN
- `NR_CAPACIDADE_MAXIMA` INT
- `CD_ENDERECO` BIGINT FK → TB_ENDERECO(`CD_ENDERECO`)

#### TB_EVENTO
- PK: `CD_EVENTO` (BIGSERIAL)
- `CD_OWNER` BIGINT FK → TB_USUARIO(`CD_USUARIO`)
- `CD_LOCAL` BIGINT FK → TB_LOCAL(`CD_LOCAL`)
- `NM_SUBTITULO` VARCHAR(200)
- `DS_DESCRICAO` TEXT
- `DS_IMAGEM_PRINCIPAL` VARCHAR(500)
- `DT_DATA_INICIO` TIMESTAMPTZ
- `DT_DATA_FIM` TIMESTAMPTZ
- `DT_INICIO_VENDAS` TIMESTAMPTZ
- `DT_FIM_VENDAS` TIMESTAMPTZ
- `NM_FORMATO` VARCHAR(50)
- `NR_VISIBILIDADE` VARCHAR(50)
- `DS_STATUS` VARCHAR(50)
- `NR_LIMITE` INT
- `DS_TAGS` TEXT
- `DT_METADATA_PUBLICACAO` TIMESTAMPTZ

#### TB_EVENTO_CATEGORIA
- PK: (`CD_EVENTO`, `CD_CATEGORIA`)
- `CD_EVENTO` BIGINT FK → TB_EVENTO(`CD_EVENTO`)
- `CD_CATEGORIA` BIGINT FK → TB_CATEGORIA(`CD_CATEGORIA`)

#### TB_SECAO
- PK: `CD_SECAO` (BIGSERIAL)
- `CD_EVENTO` BIGINT FK → TB_EVENTO(`CD_EVENTO`)
- `NM_NOME` VARCHAR(120)
- `DS_DESCRICAO` TEXT

#### TB_LOTE
- PK: `CD_LOTE` (BIGSERIAL)
- `CD_SECAO` BIGINT FK → TB_SECAO(`CD_SECAO`)
- `NM_NOME` VARCHAR(200)
- `NR_QUANTIDADE_TOTAL` INT
- `VL_VALOR_UNITARIO` NUMERIC(12,2) NOT NULL
- `DT_DATA_INICIO` TIMESTAMPTZ
- `DT_DATA_FIM` TIMESTAMPTZ

#### TB_INGRESSO
- PK: `CD_INGRESSO` (BIGSERIAL)
- `CD_LOTE` BIGINT FK → TB_LOTE(`CD_LOTE`)
- `DT_RESERVADO_EM` TIMESTAMPTZ
- `NM_STATUS` VARCHAR(30)
- `CD_CODIGO` VARCHAR(50)
- `DT_CHECKIN` TIMESTAMPTZ

#### TB_PEDIDO
- PK: `CD_PEDIDO` (BIGSERIAL)
- `CD_USUARIO` BIGINT FK → TB_USUARIO(`CD_USUARIO`)
- `NM_STATUS_PEDIDO` VARCHAR(40)
- `CD_CODIGO` VARCHAR(50)
- `VL_VALOR_TOTAL` NUMERIC(12,2)
- `VL_DESCONTO_TOTAL` NUMERIC(12,2)
- `DT_DATA_INICIO` TIMESTAMPTZ
- `DT_DATA_PAGAMENTO` TIMESTAMPTZ
- `DT_DATA_CANCELAMENTO` TIMESTAMPTZ
- `DT_DATA_FINALIZACAO` TIMESTAMPTZ

#### TB_ITEM_PEDIDO
- PK: `CD_ITEM_PEDIDO` (BIGSERIAL)
- `CD_PEDIDO` BIGINT FK → TB_PEDIDO(`CD_PEDIDO`)
- `CD_INGRESSO` BIGINT FK → TB_INGRESSO(`CD_INGRESSO`)
- `NM_STATUS` VARCHAR(30)
- `VL_VALOR_UNITARIO` NUMERIC(12,2)

#### TB_TENTATIVA_PAGAMENTO
- PK: `CD_TENTATIVA_PAGAMENTO` (BIGSERIAL)
- `CD_PEDIDO` BIGINT FK → TB_PEDIDO(`CD_PEDIDO`)
- `NM_TIPO_PAGAMENTO` VARCHAR(30)
- `DT_DATA_TENTATIVA` TIMESTAMPTZ
- `CD_REFERENCIA_GATEWAY` VARCHAR(120)
- `NM_STATUS` VARCHAR(30)
- `DS_MENSAGEM_ERRO` TEXT

#### TB_REEMBOLSO
- PK: `CD_REEMBOLSO` (BIGSERIAL)
- `CD_TENTATIVA_PAGAMENTO` BIGINT FK → TB_TENTATIVA_PAGAMENTO(`CD_TENTATIVA_PAGAMENTO`)
- `VL_VALOR` NUMERIC(12,2) NOT NULL
- `DS_MOTIVO` VARCHAR(120)
- `CD_REFERENCIA_GATEWAY` VARCHAR(120)
- `NM_STATUS` VARCHAR(30)
- `DT_CRIADO_EM` TIMESTAMPTZ
- `DT_ATRIBUIDO_EM` TIMESTAMPTZ
- `DT_FIM_PROCESSAMENTO` TIMESTAMPTZ
- `DS_INFO_GATEWAY` JSONB

#### TB_USUARIO_EVENTO
- PK: `CD_USUARIO_EVENTO` (BIGSERIAL)
- `CD_USUARIO` BIGINT FK → TB_USUARIO(`CD_USUARIO`)
- `CD_EVENTO` BIGINT FK → TB_EVENTO(`CD_EVENTO`)
- `NM_ROLE` VARCHAR(30)
- `CD_PEDIDO` BIGINT FK → TB_PEDIDO(`CD_PEDIDO`)
...
### Regras de Integridade Referencial (principais FKs)
- TB_EVENTO.`CD_OWNER` → TB_USUARIO.`CD_USUARIO`
- TB_EVENTO.`CD_LOCAL` → TB_LOCAL.`CD_LOCAL`
- TB_EVENTO_CATEGORIA.`(CD_EVENTO, CD_CATEGORIA)` → (TB_EVENTO, TB_CATEGORIA)
- TB_SECAO.`CD_EVENTO` → TB_EVENTO.`CD_EVENTO`
- TB_LOTE.`CD_SECAO` → TB_SECAO.`CD_SECAO`
- TB_INGRESSO.`CD_LOTE` → TB_LOTE.`CD_LOTE`
- TB_ITEM_PEDIDO.`CD_PEDIDO` → TB_PEDIDO.`CD_PEDIDO`
- TB_ITEM_PEDIDO.`CD_INGRESSO` → TB_INGRESSO.`CD_INGRESSO`
- TB_PEDIDO.`CD_USUARIO` → TB_USUARIO.`CD_USUARIO`
- TB_TENTATIVA_PAGAMENTO.`CD_PEDIDO` → TB_PEDIDO.`CD_PEDIDO`
- TB_REEMBOLSO.`CD_TENTATIVA_PAGAMENTO` → TB_TENTATIVA_PAGAMENTO.`CD_TENTATIVA_PAGAMENTO`
- TB_USUARIO_EVENTO.`(CD_USUARIO, CD_EVENTO)` → (TB_USUARIO, TB_EVENTO)
- TB_LOCAL.`CD_ENDERECO` → TB_ENDERECO.`CD_ENDERECO`
- TB_USUARIO.`(CD_METADATA, CD_ENDERECO)` → (TB_METADATA, TB_ENDERECO)

> Observações:
> - Tipos compatíveis com PostgreSQL e nomes seguindo os prefixos solicitados.
> - Ajuste de tamanhos e constraints (UNIQUE, CHECK) conforme regras de negócio.

![WhatsApp Image 2025-11-05 at 7 23 24 PM](https://github.com/user-attachments/assets/dd812376-65c8-434a-93e6-197e06ab54ec)
